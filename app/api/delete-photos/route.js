  ...
  try {
    ...
    if (!sessionId || !Array.isArray(files) || files.length === 0) {
      return NextResponse.json(
        { error: "sessionId and files[] are required" },
        { status: 400 }
      );
    }
    ...
    for (const f of all) {
      if (setToDelete.has(f)) {
        try {
          await unlink(path.join(sessionDir, f));
        } catch {}
      } else {
        kept.push(f);
      }
    ...
    const tmpFiles = (await readdir(sessionDir))
      .filter((f) => f.endsWith(tmpSuffix))
      .sort();
    const newNames = [];
    for (let i = 0; i < tmpFiles.length; i++) {
      ...
      try {
        ...
        newNames.push(targetName);
      } catch {}
    }

    return NextResponse.json({
      message: "Deleted and reindexed",
      files: newNames,
